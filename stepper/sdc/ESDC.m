function [ts,ys,tcpu,tcpuc] = ESDC(L,N,tspan,y0,Nt,options)
...
%               tau                 - nx1 array which contains normalized
%                                     quadrature points
% RETURNS
%   ys      - solution values
...
tic;
[IM,P01] = initW(transpose(L(:)),h,tau);
...
for i=1:Nt
    %sub stage
    for j=1:n-1
        yn(j,1,:) = N(t0 + h*tau(j),squeeze(y_stage(j,1,:)),params);
        y_stage(j+1,1,:) = P01(j,1,:).*y_stage(j,1,:) + P01(j,2,:).*yn(j,1,:);
    end
    yn(n,1,:) = N(t0 + h*tau(n),squeeze(y_stage(n,1,:)),params);
    for k=1:m
        ...
        for j=2:n-1
            n_new = reshape(N(t0+h*tau(j),squeeze(y_stage(j,1,:)),params), [1, 1, Nx]);
